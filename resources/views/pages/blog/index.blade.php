@extends('layouts.app', ['title' => 'Blog & Artikel Edukasi'])
...
@section('content')
    ...
    <div class="bg-white py-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if($posts->count() > 0)
                @php
                    // Ambil post pertama sebagai featured post
                    $featuredPost = $posts->shift();
                @endphp
                <section class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-20">
                    <div data-aos="fade-right" class="group rounded-xl overflow-hidden shadow-lg">
                        <a href="{{ route('blog.show', $featuredPost->slug) }}">
                            <img src="{{ $featuredPost->image_url }}" alt="{{ $featuredPost->title }}" class="w-full h-96 object-cover transition-transform duration-300 group-hover:scale-105">
                        </a>
                    </div>
                    <div data-aos="fade-left" data-aos-delay="200">
                        ...
                        <h2 class="mt-2 text-3xl font-extrabold text-gray-900 leading-tight">
                            <a href="{{ route('blog.show', $featuredPost->slug) }}" class="hover:text-blue-700 transition">{{ $featuredPost->title }}</a>
                        </h2>
                        <p class="mt-4 text-lg text-gray-600">{{ $featuredPost->excerpt }}</p>
                        <div class="mt-6">
                            <a href="{{ route('blog.show', $featuredPost->slug) }}" class="px-8 py-3 bg-gradient-to-r from-blue-600 to-blue-800 text-white font-bold rounded-lg shadow-lg transform hover:scale-105 hover:from-blue-700 hover:to-blue-900 transition-all duration-300">
                                Baca Selengkapnya
                            </a>
                        </div>
                    </div>
            ...
            @if($posts->count() > 0)
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($posts as $index => $post)
                        <div data-aos="fade-up" data-aos-delay="{{ $index * 100 }}" class="group bg-white rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 flex flex-col">
                            <a href="{{ route('blog.show', $post->slug) }}" class="block overflow-hidden">
                                <img src="{{ $post->image_url }}" alt="{{ $post->title }}" class="w-full h-56 object-cover transition-transform duration-300 group-hover:scale-105">
                            </a>
                            <div class="p-6 flex flex-col flex-grow">
                                <p class="text-sm text-gray-500">{{ $post->published_at->format('d F Y') }}</p>
                                <h3 class="mt-2 text-xl font-bold text-gray-800 flex-grow">
                                    <a href="{{ route('blog.show', $post->slug) }}" class="hover:text-blue-600 transition">{{ $post->title }}</a>
                                </h3>
                                <div class="mt-4">
                                    <a href="{{ route('blog.show', $post->slug) }}" class="font-semibold text-blue-600 hover:text-blue-800">Baca Selengkapnya &rarr;</a>
                                </div>
                            </div>
                        </div>
                    @endforeach
