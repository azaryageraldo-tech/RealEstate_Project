...
<aside class="w-64 bg-slate-800 text-white flex flex-col flex-shrink-0">
    ...
    <nav class="mt-8 flex-grow">
        ...
        <a href="{{ route('admin.users.index') }}" @class([
            ...
            'text-slate-400 hover:bg-slate-700 hover:text-white' => !str_contains(
                $routeName,
                'admin.users'),
        ])>
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a href="{{ route('admin.posts.index') }}" @class([
            ...
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 11a2 2 0 01-2-2V7a2 2 0 00-2-2H9a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2z">
                </path>
            </svg>
        ...
        <a href="{{ route('admin.banners.index') }}" @class([
            'flex items-center gap-4 px-6 py-4 transition',
            'bg-blue-600 text-white' => str_contains($routeName, 'admin.banners'),
            'text-slate-400 hover:bg-slate-700 hover:text-white' => !str_contains(
                $routeName,
                'admin.banners'),
        ])>
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l-1.586-1.586a2 2 0 010-2.828L14 8m-2 2l-1.586 1.586a2 2 0 01-2.828 0L6 10m12 6h2m-2 0h-2m-2 0h2m-2 0h2M3 20h18a2 2 0 002-2V6a2 2 0 00-2-2H3a2 2 0 00-2 2v12a2 2 0 002 2z">
                </path>
            </svg>
    ...
    <div class="p-6 border-t border-slate-700">
        <a href="{{ route('profile.edit') }}" class="flex items-center gap-4 group">
            <img class="w-10 h-10 rounded-full object-cover"
                src="{{ Auth::user()->profile_photo_path ? Storage::url(Auth::user()->profile_photo_path) : 'https://i.pravatar.cc/150?u=' . Auth::id() }}"
                alt="Avatar">
            <div class="flex-grow">
        ...
        <form method="POST" action="{{ route('logout') }}" class="mt-6">
            @csrf
            <button type="submit"
                class="w-full flex items-center gap-4 px-4 py-3 bg-red-600/20 text-red-400 rounded-lg hover:bg-red-600 hover:text-white transition">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
